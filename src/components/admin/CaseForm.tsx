...
const CaseForm = ({ caseId }: CaseFormProps) => {
  ...
  const loadCaseData = async () => {
    ...
    try {
      ...
      if (needsData && needsData.length > 0) {
        ...
        setMonthlyNeeds(formattedNeeds);
      }
  ...
  const onSubmit = async (data: CaseFormData) => {
    ...
    try {
      ...
      toast({
        title: "خطأ",
        description: caseId ? "حدث خطأ أثناء تحديث الحالة" : "حدث خطأ أثناء إضافة الحالة",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div className="flex justify-end">
        <Button type="submit" disabled={loading || loadingData} size="lg">
          {loading ? (caseId ? "جار التحديث..." : "جار الحفظ...") : (caseId ? "تحديث الحالة" : "حفظ الحالة")}
        </Button>
      </div>
    </form>
  );
